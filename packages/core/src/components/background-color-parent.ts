 ...
 * @returns {string}
 */
export function getBackgroundColorThroughParents(element: HTMLElement): string {
  let parentBackgroundColor = '';
  while (parentBackgroundColor === '' && element.parentElement) {
    let color = window.getComputedStyle(element.parentElement, null).getPropertyValue('background-color');
